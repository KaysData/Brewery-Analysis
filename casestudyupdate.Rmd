...
names(brewNBeer)= c("Brewery_id","Brewery_Name", "Beer_ID" ,"ABV" , "IBU",  "Style","Ounces","Beer_Name" ,"City","State" )
kable(head(brewNBeer),align = params$kable_align) %>% kable_styling( position = "left")

colnames(brewNBeer)[colnames(brewNBeer)=="Brewery_id"] <- "Brewery_ID"
colnames(brewNBeer)[colnames(brewNBeer)=="Brewery_Name"] <- "Beer_Name1"
colnames(brewNBeer)[colnames(brewNBeer)=="Beer_Name"] <- "Brewery_Name"
colnames(brewNBeer)[colnames(brewNBeer)=="Beer_Name1"] <- "Beer_Name"
...
kable(colSums(is.na(brewNBeer)),align = params$kable_align, col.names = c('Column Name \t  # NA')) %>% kable_styling()
...
We looked to see if we could find a relationship between bitterness and alcohol content by producing a scatterplot. More alcoholic beers had a the full range of bitterness, but less alcoholic beers tended to have low bitterness. 
...
``` {r echo =params$echo }
t.test(brewNBeer[brewNBeer$State=='TX',]$ABV, 
       brewNBeer[brewNBeer$State=='CO',]$ABV, 
       var.equal = TRUE,
       #paired=TRUE, 
       conf.level=0.95)

t.test(brewNBeer[brewNBeer$State=='TX',]$ABV, 
       brewNBeer[brewNBeer$State=='OR',]$ABV,  
       #paired=TRUE, 
       var.equal = TRUE,
       conf.level=0.95)
